...
#inputs = aoc.examples(2024, 13, lines=True)[0]  # correct answer: only on 2nd and 4th
inputs = aoc.get_input(2024, 13, lines=True)
...
for m in machines:
    A, B = sympy.symbols('A B')
    eqs = [ sympy.Eq( m[0][i]*A + m[1][i]*B, m[2][i] + 10_000_000_000_000 ) for i in range(2)]
    solution = sympy.solve(eqs, (A, B))
    print(eqs, solution)
    if all(isinstance(x, sympy.core.numbers.Integer) for x in solution.values()):
        total_price += solution[A]*3+solution[B]
...
# submit answer
aoc.submit(2024, 13, 2, total_price)
